  ...
  HistoryBloc(this.goodsIssueUseCase, this.departmentUsecase,
    ...
    on<GetExportItemByWarehouseEvent>((event, emit) async {
      ...
      try {
        ...
        emit(GetExportItemByWarehouseFailState(DateTime.now(), ErrorPackage('')));
      }
    });
    ...
    on<GetAllInfoImportEvent>((event, emit) async {
      ...
      try {
        final item = await itemUsecase.getAllItem();
        final warehouse = await locationUsecase.getAllWarehouse();
        final goodReceipt = await goodsReceiptUseCase.getAllGoodReceipt();
        emit(GetAllInfoImportSuccessState(
            DateTime.now(), warehouse, item, goodReceipt));
      } catch (e) {
    ...
    on<GetImportItemByWarehouseEvent>((event, emit) async {
      ...
      try {
        ...
        emit(GetImportItemByWarehouseFailState(DateTime.now(), ErrorPackage('')));
      }
    });
